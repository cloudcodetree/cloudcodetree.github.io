...
/**
 * Bespoke illustration of batched inference: many in-flight requests are grouped
 * into one model pass, then results return together — far higher GPU throughput
 * than one-at-a-time. Request dots flow in, gather into a batch, pass, and exit.
 * Static-safe.
 */
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: '0 0 auto' }}>
          {REQS.map((i) => (
            ...
            >
              ...
              <span style={{ fontFamily: MONO, fontSize: 10, color: '#8b98a8' }}>req {i + 1}</span>
            </motion.div>
          ))}
        </div>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 3, justifyContent: 'center', marginTop: 6 }}>
            {REQS.map((i) => <span key={i} style={{ width: 7, height: 7, borderRadius: 2, background: accent }} />)}
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: '0 0 auto' }}>
          {REQS.map((i) => (
            ...
            >
              ...
              <span style={{ fontFamily: MONO, fontSize: 10, color: '#8b98a8' }}>resp {i + 1}</span>
            </motion.div>
          ))}
        </div>
      </div>
